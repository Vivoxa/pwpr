...
RUN apk --update add --no-cache bash ruby ruby-bigdecimal ruby-irb ruby-bundler grep less curl zlib \
&& rm -rf /var/cache/apk/* \
&& gem install -N rake -v 10.4.2 -- --use-system-libraries \
&& set -ex \
&& apk --update add nodejs mariadb-client-libs mariadb-client mariadb-libs \
