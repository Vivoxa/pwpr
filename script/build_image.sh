...
set -e
    ...
    if [[ $@ == *"--preprod"* ]] ;then
      ...
      echo '********************************************************************************************************************************************'
      echo '             Building the command to run on the container'
      command="RAILS_ENV=preprod bundle && RAILS_ENV=preprod bundle exec rake db:drop db:create db:scheme:load db:migrate db:seed && RAILS_ENV=preprod bundle exec rails s -p 3000 -b '0.0.0.0'"
      echo $command
      echo '********************************************************************************************************************************************'
      echo ''
      docker build --build-arg APP_DIR=pwpr --build-arg  COMMAND="$command" -t=pwpr .
