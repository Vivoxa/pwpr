        ...
        context 'when calling create' do
          context 'with correct params' do
            it 'expects a 200 response status' do
              params = {scheme_operator: {approved: true, password: 'my_password', email: 'star@star.com', first_name: 'star', scheme_ids: [1]}}
              post :create, params
          ...
          context 'with scheme_id missing' do
            it 'expects validation to fail' do
              ...
              params = {scheme_operator: {approved: true, password: 'my password', email: 'myemail@pwpr.com', first_name: 'star', scheme_ids: []}}
              post :create, params
    ...
    context 'when Admin has super_admin role' do
      ...
      context 'when calling create' do
        it 'expects a 302 response status' do
          params = {scheme_operator: {invitation_sent_at: DateTime.now, password: 'my_password', email: 'star@star.com', first_name: 'star', scheme_ids: [1]}}
          post :create, params
          expect(response.status).to eq 302
          user = SchemeOperator.find_by_email('star@star.com')
          expect(user).to be_a(SchemeOperator)
