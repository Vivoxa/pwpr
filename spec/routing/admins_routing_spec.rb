...
RSpec.describe AdminsController, type: :routing do
  describe 'routing' do
    ...
    it 'routes to #permissions via GET' do
      expect(get('/admins/1/permissions')).to route_to('admins#permissions', admin_id: '1')
    end

    it 'routes to #update_permissions via PUT' do
      expect(put('/admins/1/update_permissions')).to route_to('admins#update_permissions', admin_id: '1')
    end
  end
end
